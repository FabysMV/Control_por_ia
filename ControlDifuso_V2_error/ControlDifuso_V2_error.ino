...
//---------------------Variables Varias------------------------
int Yconv = 0;
//int C[10] = {-2900, -1000, 1000, -2900, -1000, -1000, -1000, 1000, 1000, 2900};
int C[10] = {-1050, -375, 375, -1050, -375, -375, -375, 375, 375, 1050};
float Kn[10];//Arreglo donde se almacenara los valores de K
float ydefuzzden[10];
float ydefuzznum[10];
float yden,ynum;
int ydefuzz;
...
void Fuzzy(int angle)
{
  ...
  float n = exp(-0.00024691358*pow(((-1*angle) - (-180)), 2));
  float e2 = exp((-0.00103305785*pow((angle-(-45)),2)));
  float mn = exp((-0.00103305785*pow(((-1*angle)-(-45)),2)));
  float e3 = exp((-0.00103305785*pow((angle-45),2)));
  float mp = exp((-0.00103305785*pow(((-1*angle)-45),2)));
  float e4 = exp((-0.00024691358*pow((angle-180),2)));
  float p = exp((-0.00024691358*pow(((-1*angle)-180),2)));
//************************************************************************************
  ...
  for (int i=0; i<10; i++){ // Sumatoria del numerador
    ynum += ydefuzznum[i];
  }
